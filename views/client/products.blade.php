                            ...
                            <div class="card h-100">
                                ...
                                <div class="card-body p-4">
                                    <div class="text-center">
                                        <!-- Product name-->
                                        <h5 class="fw-bolder"><a href="/detail/{{$product['p_id']}}">{{$product['p_name']}}</a></h5>
                                        <!-- Product price-->
                                        <span class="text-muted text-decoration-line-through">{{$product['p_price']}}</span>
                ...
                <ul class="pagination">
                    @for ($i = 1; $i <= $products['totalPage']; ++$i)
                        <li class="page-item @if ($products['page'] == $i) active @endif">
                            <a class="page-link" href="/products/?page={{ $i }}&limit={{ $products['limit'] }}">{{ $i }}</a>
                        </li>
                    @endfor
                </ul>
            </nav>
